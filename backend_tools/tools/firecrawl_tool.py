    ...
    async def execute(self, parameters: FirecrawlInput) -> dict:
        try:
            ...
            if mode == 'scrape':
                ...
                # crawl API 返回一个 CrawlJob 对象
                job = firecrawl_client.crawl(
                    url=validated_params.url,
                    limit=validated_params.limit,
                    scrape_options=validated_params.scrape_options
                )
                # 从 CrawlJob 对象中提取 job_id
                job_id = getattr(job, 'id', None) or getattr(job, 'job_id', None)
                if not job_id and hasattr(job, '__dict__'):
                    # 尝试从对象属性中查找
                    for key, value in job.__dict__.items():
                        if 'id' in key.lower() and isinstance(value, str):
                            job_id = value
                            break
                result = {"status": "crawl job started", "job_id": job_id or str(job)}
            elif mode == 'map':
                ...
                # extract API 返回一个任务对象
                job = firecrawl_client.extract(
                ...
                if not job_id and hasattr(job, '__dict__'):
                    for key, value in job.__dict__.items():
                        if 'id' in key.lower() and isinstance(value, str):
                            job_id = value
                            break
                result = {"status": "extract job started", "job_id": job_id or str(job)}
            elif mode == 'check_status':
                validated_params = CheckStatusParams(**params)
                # 优先使用 V2 版本中更名后的方法
                if hasattr(firecrawl_client, 'get_crawl_status'):
                    result = firecrawl_client.get_crawl_status(validated_params.job_id)
                # 保留对旧方法名的尝试，以增强兼容性
                elif hasattr(firecrawl_client, 'check_crawl_status'):
                    result = firecrawl_client.check_crawl_status(validated_params.job_id)
                else:
                    return {"success": False, "error": "Firecrawl client does not have get_crawl_status or check_crawl_status method."}
            else:
