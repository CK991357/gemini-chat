...
export default {
  async fetch(request) {
    ...
    if (pathname.startsWith("/ws-gemini25/google.ai.generativelanguage.v1beta.GenerativeService.BidiGenerateContent")) {
...
class MultimodalLiveClientWorker extends EventEmitter {
  ...
  async sendContentToGemini(parts, turnComplete) {
    ...
    const url = `${BASE_URL}/${API_VERSION}/${this.config.model}:streamGenerateContent`;
    try {
      const response = await fetch(url, {
        method: "POST",
