...
us_stock: {
    ...
    config: {
        ...
        structure: [
            ...
            "## 四、财务深度分析 (In-Depth Financial Analysis)",
            "以下分析基于[公司名称]截至2025财年的完整历史财务数据。基础数据来源：AlphaVantage；比率数据来源：经整理的财务比率表。",
            "### 4.1 基础财务数据概览",
            "   - [以表格形式展示近5年的核心财务数据，至少包括：营收、净利润、自由现金流、各年度同比增长率。数据均来源于上传的JSON文件。]",
            "   - [表格格式应清晰规范，数值右对齐，可参考如下样式（仅为格式示例，实际数据需从JSON文件中提取）：]",
            "     | 财年 | 营收（亿美元） | 净利润（亿美元） | 自由现金流（亿美元） | 营收同比增长 | 净利润同比增长 | 自由现金流同比增长 | ... |",
            "     |------|----------------|------------------|----------------------|--------------|----------------|--------------------|-----|",
            "     | 2025 | 4162           | 1120             | 988                  | +6.4%        | +19.5%         | -9.2%              | ... |",
            "     | 2024 | 3910           | 937              | 1088                 | +2.0%        | -3.4%          | +9.3%              | ... |",
            "     | 2023 | 3833           | 970              | 996                  | -2.8%        | -2.8%          | -10.6%             | ... |",
            "     | 2022 | 3943           | 998              | 1114                 | +7.5%        | +5.4%          | +19.9%             | ... |",
            "     | 2021 | 3658           | 947              | 929                  | +33.4%       | +64.9%         | +26.7%             | ... |",
            "   - [上表为核心指标示例，实际报告中可根据数据可获得性和行业特点增加其他关键指标列（如研发费用占比、净息差、同店销售增长率等），最后一列的“...”示意可扩展性。]",
            "   - [此外，根据公司所在行业特性，可补充其他关键基础指标（如科技行业的研发费用占比、金融行业的净息差、零售业的同店销售增长率等），并以表格或列表形式呈现。]",
            "### 4.2 核心财务比率解读",
            "   - [基于上传Markdown文件中的五大类表格（盈利能力、流动性、杠杆、效率、现金流），对每个类别进行深入的解读，撰写详细的分析段落，并完整呈现这些表格。]",
            "   - [在解读每个比率时，必须引用之前研究步骤中收集到的行业数据（如市场增长率、竞争对手财报、监管动态等）来支撑分析，并标注来源。]",
            "## 五、经营变化趋势分析 (Trend & Change Analysis)",
            "   - [计算关键财务指标的年增长率或变化百分比，分析其变化趋势。]",
            "   - [识别改善或恶化的关键领域，探讨变化背后的可能原因（如产品涨价、成本控制、市场竞争等）。]",
            "   - [制作趋势表格，展示近3-5年各项指标的变化情况。]",
            "   - [结合行业背景，判断公司经营状况是在向好、稳定还是向差。]",
            "## 六、风险因素 (Risk Factors)",
            "   - [需涵盖宏观、行业、公司特定风险，可结合搜索获取的最新信息。]",
            "## 七、综合结论与经营评价 (Conclusion & Business Assessment)",
...
- **必须充分利用用户上传的两个财务数据文件**（均已存入数据总线 DataBus，键名分别为 \`upload_1_financial_ratio_result_json\` 和 \`upload_2_[公司代码]_report_md\`，例如 \`upload_2_AAPL_report_md\`）。其中：
  - **基础数据文件（JSON）**：包含公司多年的基础财务数据，如营收、净利润、自由现金流、增长率、分部门数据（若有）、历史年度数据等。这些数据是4.1节分析的核心。
  - **比率数据文件（Markdown）**：包含五大类历史财务比率表格（盈利能力、流动性、杠杆、效率、现金流），用于4.2节的深入解读。
...
- **提取并展示基础数据**：
  ...
  - **行业特定指标**（根据行业从JSON中选取合适的基础数据，若JSON中无则留空，可结合搜索补充）：
    - **科技行业**：研发费用占营收比（若JSON中有，否则可结合搜索）、服务业务收入占比（若公司有披露）、资本开支强度等。
    - **金融行业**：净息差、不良贷款率、资本充足率、ROA等（若JSON中有对应指标）。
    - **能源行业**：桶油成本、储量替代率、资本回报率等（若JSON中有）。
    - **消费品行业**：同店销售增长率、门店数量变化、线上收入占比等（若JSON中有）。
    - **医药行业**：研发管线进展、专利到期情况、重磅药品销售额（若JSON中有，否则需搜索）。
  - **历史趋势数据**：选取近3-5年的营收、净利润、自由现金流，以简洁表格或列表形式展示，直观反映规模变化。
- **数据解读要求**：
  - 对每个指标进行文字解读，解释其数值含义、近年变化趋势，并**必须结合行业背景**（如行业增速、竞争格局、技术变革）说明驱动因素。
  - 例如：若营收增长率高于行业平均水平，可归因于产品创新或市场份额提升；若自由现金流强劲，可结合资本开支周期分析。

#### 2.2 核心财务比率解读（4.2节）
- **完整呈现表格**：从Markdown文件中提取五大类比率表格（盈利能力、流动性、杠杆、效率、现金流），确保格式清晰，保留表格内容。
- **撰写分析段落**：对每个类别进行深入解读，分析指标变化趋势及其背后的业务原因，并**必须引用行业信息**（如对比行业平均水平、竞争对手数据）来增强说服力。
- **引用标注**：引用行业数据（如历史数据、行业报告、行业动态等）或外部信息（如竞争对手数据、新闻等），以支持分析，并标注来源。

### 3. 经营变化趋势分析指引
- **计算变化率**：对于每个关键指标，计算最近年份相对于前几年的年增长率或变化百分比（例如，近三年ROE的复合年增长率）。
...
- 例如，如果公司是周期股，可重点分析宏观经济和供需；如果是成长股，可侧重市场空间和产品创新；如果是价值股，可关注股息和回购。
- 确保报告逻辑连贯，像一位专业分析师撰写。
`
    },
