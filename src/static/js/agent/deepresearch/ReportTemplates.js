...
`
    ...
    translation_extraction: {
        name: "网站内容完整翻译报告",
        description: "忠实于网站原文的完整翻译，包含表格、图片等所有原始信息",
        config: {
            // 🔥 固定结构，不启用动态结构
            ...
            wordCount: "动态调整（以原文长度为基准）",
            style: "客观、准确、结构化、无观点、纯信息整理",
...
### 1. 绝对忠实原则
- **禁止添加任何模型的观点、评论、补充信息**
- **禁止对原文内容进行任何形式的美化、简化或润色**
- **禁止删除原文中的任何信息**
- **禁止对原文立场、观点进行调整**

### 2. 完整翻译标准
- **全文翻译**：必须提供完整的段落级翻译
- **结构保持**：完全保持原文的段落结构、标题层级和逻辑顺序
- **术语一致**：专业术语在整个报告中保持一致的翻译
- **文化适应**：文化特定概念需添加简要解释，但不改变原意

### 3. 特殊内容处理
- **表格处理**：转换为Markdown表格格式，保持原表格结构
- **图片处理**：提取并翻译所有图片描述文本
- **代码处理**：技术文档中的代码块保留原格式，不翻译
- **链接处理**：保持链接原样，只翻译链接文本

### 4. 质量控制
- **完整性检查**：确保没有遗漏原文的任何部分
- **一致性检查**：术语翻译前后一致
- **准确性验证**：关键信息（日期、数字、名称）必须准确无误
- **格式统一**：整个报告使用统一的格式标准
`
        },
        researchMode: "translation_extraction"
    }
};
...
export function getTemplateByResearchMode(researchMode) {
    const modeToTemplate = {
        ...
        'technical': 'technical',
        'standard': 'standard',
        'translation': 'translation_extraction' // 🎯 新增：翻译模式映射
    };
