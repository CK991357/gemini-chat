...
us_stock: {
    ...
    config: {
        ...
        wordCount: "4000-8000字（以分析彻底性为准）",
        style: "投行级、数据驱动、逻辑严谨、前瞻性",
        requirements: `
## 🎯 美股深度研究核心要求

### 1. 数据驱动原则 (Data-Driven Principle)
- **必须充分利用用户上传的财务数据**（通常为JSON或Markdown文件）。这些数据已存入数据总线(DataBus)，键名以 \`upload_\` 开头。
- 上传的Markdown文件（如 \`AAPL_report.md\`）包含了公司多年的财务比率历史（盈利能力、流动性、杠杆、效率、现金流等五大类表格）。
- **在报告中，必须完整保留这些表格**（可直接从Markdown中提取），它们是财务分析的基础。表格的格式已优化为标准的Markdown表格，便于阅读。
- 所有财务数据点必须标注来源（引用上传文件的来源编号，如[1]）。
- 对于外部信息（行业数据、竞争对手信息、新闻等），必须使用tavily_search或crawl4ai获取，并标注来源。

### 2. 财务深度分析指引
- 在呈现历史财务比率表格后，需要对每个类别进行深入的**文字分析**：
  - **盈利能力**：分析ROE、ROA、毛利率的变化趋势，找出驱动因素（如成本控制、定价能力）。
  - **流动性**：评估短期偿债能力，关注现金比率和营运资本的变化。
  - **杠杆**：分析负债水平是否合理，利息保障倍数是否安全。
  - **效率**：解读资产周转率、存货周转天数的变化，反映运营效率。
  - **现金流**：自由现金流的趋势及其与净利润的关系，判断盈利质量。
- 如果可能，将关键比率与行业平均水平进行对比（可通过搜索获取行业基准）。
