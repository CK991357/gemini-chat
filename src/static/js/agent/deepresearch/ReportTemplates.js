...
`
    ...
    data_mining: {
        name: "纯数据挖掘报告",
        description: "专注于收集、清洗和整理互联网上的原始数据，支持多种数据格式和场景。",
        config: {
            dynamic_structure: true,
            ...
            structure: [
                "# [智能生成的标题]",
                "## 数据报告概述",
                "   - [本报告专注于原始数据收集，不包含分析解读]",
                "   - [数据已按照统一标准进行清洗和格式化]",
                "## 一、核心数据汇总",
                "   - [根据数据特点自动选择呈现方式]",
                "   - [支持表格、列表、时间线、地理坐标等多种格式]",
                "   - [自动识别并标注数据来源和质量等级]",
                "## 二、数据质量评估",
                "   - [完整性评估：数据覆盖度分析]",
                "   - [一致性评估：冲突数据识别]",
            ...
            wordCount: "1000-5000字（根据数据量动态调整）",
            style: "客观、准确、结构化、可机读",
...
| 数据类型 | 支持格式 | 输出要求 |
|----------|----------|----------|
| **结构化数据** | Markdown表格、CSV片段、JSON片段 | 字段名统一，单位标准化 |
| **时间序列** | 时间线表格、折线图数据点 | 时间格式统一，间隔一致 |
| **分类数据** | 对比表格、矩阵图、雷达图数据 | 分类维度清晰，覆盖完整 |
| **地理数据** | 坐标表格、区域代码、地图数据 | 坐标格式统一，区域标准 |
| **数值数据** | 统计数据、分布数据、范围数据 | 数值精度一致，范围明确 |
| **文本数据** | 关键词列表、短语集、文档摘要 | 文本清洗，去除噪音 |
...
**完整性等级**（自动评估）：
- ✅ **A级**：数据完整，来源可靠，格式统一
- ⚠️ **B级**：数据基本完整，来源一般，格式基本统一
- ❌ **C级**：数据缺失严重，来源单一，格式混乱

**验证要求**：
1. 至少2个独立来源验证关键数据
2. 数据采集时间在3个月内（除非历史数据）
3. 数值数据必须有明确单位和精度
4. 文本数据必须经过基本清洗
