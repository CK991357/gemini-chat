...
`
    ...
    data_mining: {
        name: "纯数据挖掘报告",
        description: "专注于收集和整理互联网上的原始数据，以表格形式客观呈现，不包含任何AI分析观点。",
        config: {
            dynamic_structure: true,
            ...
            structure: [
                "# [基于数据主题自主生成的标题]",
                "## 数据报告说明",
                "   - [本报告仅呈现从互联网收集的原始数据，不包含任何分析、观点或解读]",
                "   - [所有数据均注明来源，可通过来源编号追溯]",
                "## 一、核心数据表格",
                "   - [这是报告的主要部分，将所有收集到的数据以表格形式系统化呈现]",
                "   - [每个表格必须有清晰的标题和表头]",
                "   - [每行数据必须标注来源编号，例如：[1, 3]]",
                "   - [数值数据使用标准格式，如：1,234.56]",
                "   - [百分比保留2位小数，如：12.34%]",
                "   - [日期统一为YYYY-MM-DD格式]",
                "   - [空值用'N/A'标注]",
                "## 二、辅助数据清单",
            ...
            wordCount: "2000-4000字（以数据量为准）",
            style: "客观、准确、结构化、无观点",
            
            requirements: `
## 📊 数据挖掘模式强制要求 (Data Mining Protocol)
...
### 数据收集规范：
1. **来源多样性**：每个数据点必须来自至少2个独立来源
2. **时效性验证**：必须注明数据发布时间/年份
3. **格式统一**：
   - 数值数据：使用标准格式（如 1,234.56）
   - 百分比：保留2位小数（如 12.34%）
   - 日期：统一为 YYYY-MM-DD 格式

### 表格组织规范：
1. **表头清晰**：明确标识数据维度和单位
2. **排序逻辑**：按时间、数值大小或字母顺序排列
3. **空值处理**：用 "N/A" 或 "未找到" 标注
4. **来源标注**：每行数据必须标明来源编号
...
| 年份 | GDP (万亿元) | 增长率 | 数据来源 |
|------|-------------|--------|----------|
| 2023 | 126.06      | 5.20%  | [1, 3]   |
| 2022 | 121.02      | 3.00%  | [2, 4]   |
\`\`\`
