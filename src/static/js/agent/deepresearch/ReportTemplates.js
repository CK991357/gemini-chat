...
us_stock: {
    ...
    config: {
        ...
        structure: [
            ...
            "   - [必须基于用户上传的“基础财务数据详表”Markdown文件（键名如 `upload_1_[公司代码]_base_financials_md`）。该文件包含公司多年的原始财务数据（损益表、资产负债表、现金流量表）、同比变化、复合年增长率（CAGR）以及详细的财务健康评分模型。]",
            "   - [必须完整呈现以下核心内容：",
            "       1. 公司基本信息：名称、所属行业。",
            ...
            "   - [对上述数据进行详细解读，解释数值含义、近年变化趋势，并必须结合行业背景说明驱动因素。详细的数据提取路径和解读要求请遵循“美股深度研究核心要求”中的指引。]",
...
### 2. 财务与行业融合的深度分析指引（适用于所有行业）
  #### 2.1 基础财务数据概览（第四部分）
- **识别公司基本信息**：从基础财务数据详表的标题和元数据中获取公司名称、股票代码、行业分类（文件开头部分）。
- **提取最新年度核心规模指标**：
  - 从“损益表核心数据”各表格中提取最新年度（如2025年）的**营收、净利润、EBITDA、研发费用**等。
  - 从“资产负债表核心数据”各表格中提取最新年度的**总资产、总负债、股东权益、营运资本**。
  - 从“现金流量表核心数据”各表格中提取最新年度的**经营现金流、资本支出、自由现金流**。
  - 从“每股数据”中提取最新年度的**每股收益**。
- **提取增长趋势指标**：
  - 从“复合年增长率 (CAGR)”部分直接获取**近3年、近5年、全部年份的CAGR**（营收、净利润、总资产、股东权益、经营现金流、自由现金流、每股收益）。
- **展示历史趋势数据（近3-5年）**：
  - 选取最近3-5年的数据，以简洁表格形式呈现**营收、净利润、自由现金流、资本支出、营运资本**，并确保同比变化率已在文件中列出（可直接引用）。
- **提取高级财务指标**：
  - **有效税率、EBITDA利润率、资本支出/折旧**：从损益表和现金流量表的相应行中直接获取最新年度数值及历史趋势。
  - **营运资本变动**：从资产负债表后的“营运资本变动”表格中获取历年变动值。
  - **财务健康评分**：从“财务健康评分模型”部分获取历年总分、评级及各维度得分与依据，并解读其对整体财务健康度的指示意义。
- **数据解读要求**：
  - 对每个指标进行文字解读，解释其数值含义、近年变化趋势，并**必须结合行业背景**（如行业增速、竞争格局、技术变革）说明驱动因素。
  - 例如：若营收CAGR高于行业平均水平，可归因于产品创新或市场份额提升；若自由现金流强劲，可结合资本开支周期分析；若财务健康评分从“健康”升至“非常健康”，需指出哪些维度改善。

  #### 2.2 核心财务比率解读（第五部分）
- **完整呈现五大类比率表格**：从财务比率历史数据文件中原样复制**盈利能力、流动性、杠杆、效率、现金流**五大类表格（保持Markdown格式）。
- **对每类比率进行深度解读**：
  - **盈利能力**：分析ROE、ROA、毛利率、净利率、ROIC的变化趋势，结合行业背景说明驱动因素（如产品定价权、成本控制、服务业务占比提升）。
  - **流动性**：评估流动比率、速动比率、现金比率的变化，解释与公司运营模式的关系（如负营运资本反映供应链议价能力）。
  - **杠杆**：分析负债权益比、资产负债率、利息保障倍数的变化，判断财务风险是否可控。
  - **效率**：解读资产周转率、存货周转天数、应收账款周转天数、应付账款周转天数、现金转换周期的变化，反映运营效率。
  - **现金流**：分析自由现金流、资本支出相关比率，评估盈利质量和再投资力度。
- **数据解读要求**：
