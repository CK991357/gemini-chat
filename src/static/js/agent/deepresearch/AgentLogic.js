...
### 🐍 Python Sandbox 特别诊断指南 (数据注入)

**系统更新通知**: 数据注入流程已升级。系统现在会自动处理  \`\"{{LAST_OBSERVATION}}\"\` 占位符，并对其内容进行安全转义，从根本上防止因外部数据包含特殊字符而导致的 \`SyntaxError\`。
**新的诊断流程**: 因此，如果你在使用了占位符后仍然遇到 \`SyntaxError\` 或 \`NameError\`，**问题极有可能出在你的代码结构上，而不是被注入的数据上**。
**请遵循以下自查清单**:
1.  **我是否忘记了给占位符加双引号?** **错误**: \`input_data = {{LAST_OBSERVATION}}\`。**正确**: \`input_data = "{{LAST_OBSERVATION}}"\`。这会导致 \`NameError\` 或 \`SyntaxError\`。
2.  **我是否“画蛇添足”地手动粘贴了数据?** **错误**: \`input_data = """...上一步很长的内容..."""\`。**正确**: 始终使用占位符，让系统来处理注入。
3.  **我是否犯了其他 Python 语法错误?** 检查代码的其他部分是否有遗漏的逗号、括号或不正确的缩进。
...
## 🧠 【强制】实现知行合一！(Python Sandbox 自我审查)
**当你的行动是 \`python_sandbox\` 时，你必须在生成代码后、最终输出前，进行一次严格的自我审查。**

### **思考阶段 (内心独白)**
1.  **陈述意图**: "我的目标是 [做什么，例如：定义数据并计算统计量]"。
2.  **诊断 (如果适用)**: "上一步的错误是 [具体错误]，原因是 [原因分析]"。
3.  **声明修复**: "我的修复方案是 [具体方案，例如：将列表 [105, ...] 赋值给 sales_data_list]"。

### **行动生成阶段 (编写代码)**
- 根据你的意图和修复方案，完整地编写出 Python 代码。

### **审查阶段 (输出前的最后检查 - 必须回答)**
- **在你的"思考:"部分的末尾，你必须添加一个"代码审查清单"并回答：**
    1.  **数据完整性**: 我是否已将所有必需的数据（例如 \`[105, 130, ...]\`）完整无误地填写到代码中了？ -> **是/否**
    2.  **修复应用**: 如果上一步失败了，我新生成的代码是否**确实**应用了我声明的修复方案？ -> **是/否**
    3.  **语法预览**: 我快速预览了我的代码，所有括号 \`()\`、赋值 \`=\`、引号 \`"\` 是否都看起来完整且配对？ -> **是/否**

**只有当所有审查问题的答案都是“是”时，你才能输出你的“行动”。如果任何一个答案是“否”，你必须重新生成代码直到满足所有条件。**

**✅ 正确的思考格式示例 (修复场景):**
思考: 我识别到上一步的错误是 \`SyntaxError\`，因为 \`sales_data_list = \` 后面缺少数据。我的修复方案是将列表 \`[105, 130, ...]\` 赋值给它。

**代码审查清单**:
1.  数据完整性: 我是否已将所有必需的数据...完整无误地填写到代码中了？ -> 是
2.  修复应用: 我新生成的代码是否确实应用了我声明的修复方案？ -> 是
3.  语法预览: 我快速预览了我的代码...是否都看起来完整且配对？ -> 是

行动: python_sandbox
行动输入: { "code": "import numpy as np\\nsales_data_list = [105, 130, ...]\\n..." }
`;
