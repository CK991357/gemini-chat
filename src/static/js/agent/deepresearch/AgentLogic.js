...
#### 单产品分析模式：
- **深度搜索**："{产品} 技术参数 材质 成分 2025"
- **竞品发现**："同价位 {产品} 替代 推荐"
- **使用反馈**："{产品} 真实使用 长期体验"

#### 直接对比模式：
- **对比搜索**："{产品A} vs {产品B} 对比 2025"
- **交叉验证**：分别搜索两个产品的独立评测
...
### 搜索质量要求：
- 必须包含时序关键词："2025"、"最新"、"当前"
- 必须验证来源权威性：官方 > 专业媒体 > 真实用户
- 必须检查数据时效性：优先最近3个月的内容
...
**场景**：品牌官网无法访问、技术参数不完整
**解决方案**：
1. **寻找官方替代源**：品牌授权经销商页面、官方社交媒体账号、品牌发布会视频/文稿
2. **多语言搜索**：尝试英文/日文等外文官方网站，使用翻译工具获取关键参数
3. **技术文档追溯**：搜索品牌技术白皮书、产品手册PDF的HTML版本

#### 第二层：专业评测缺失
**场景**：Vogue、Elle等专业评测内容较少或过时
**解决方案**：
1. **扩展评测来源**：
   - 时尚博主深度评测（1000字以上专业分析）
   - 皮肤科医生/材料专家技术分析
   - 行业研究报告数据引用
2. **国际评测翻译**：搜索英文专业评测，提取关键数据和评价
3. **相关评测替代**：寻找同系列、同品牌其他产品的评测作为参考

#### 第三层：用户反馈不足
**场景**：小红书、电商平台高质量评价数量不足
**解决方案**：
1. **平台扩展策略**：
   - 中文：小红书 > 知乎 > 微博 > B站
   - 英文：Reddit > YouTube评论 > 专业论坛
   - 日文/韩文：当地美妆/时尚论坛
2. **质量筛选标准**：
   - 优先：1000字以上 + 图片/视频 + 3个月以上使用报告
   - 其次：500字以上 + 具体优缺点 + 使用场景描述
   - 避免：简单评分、一句话评价、营销内容
3. **时间范围放宽**：从"最近3个月"放宽到"最近6个月"
...
### 数据验证与交叉检查：
- **三源验证原则**：关键参数需至少3个独立来源验证
- **权威性排序**：官方数据 > 专业机构评测 > 高质量用户反馈
- **时效性检查**：优先2025年数据，2024和2023年数据需标注时间

### 个性化建议保障：
- **基于现有信息决策**：即使信息不完整，也必须给出明确的购买建议
- **明确标注信息缺口**：在报告中明确说明哪些维度数据不足
- **提供补充建议**：建议用户如何进一步验证缺失信息

### 自适应分析策略：
根据数据丰富度动态调整报告深度：
- **数据充足（≥80%）**：进行全面深度对比分析
- **数据中等（50-80%）**：聚焦关键维度，明确数据缺口
- **数据有限（<50%）**：提供基于有限信息的初步建议，并指导后续验证

### 风险控制：
- **避免过度推断**：不在数据不足的情况下做出绝对性结论
- **保持建议的可调整性**：明确建议的置信度和可能的变化
        ...
        const fallbackPlans = {
            ...
            shopping_guide: {
                research_plan: [
                    {
                        step: 1,
                        sub_question: "解析用户需求，确定分析模式（单产品/对比/开放式）",
                        initial_queries: ["奢侈品分析框架", "用户需求解读方法"],
                        depth_required: "深度挖掘",
                        expected_tools: ["tavily_search"],
                        temporal_sensitivity: "中"
                    },
                    {
                        step: 2,
                        sub_question: "收集官方技术参数和核心数据",
                        initial_queries: ["品牌官方技术参数", "产品规格说明 2024"],
                        depth_required: "深度挖掘",
                        expected_tools: ["crawl4ai"],
                        temporal_sensitivity: "高"
                    },
                    {
                        step: 3,
                        sub_question: "搜索专业评测和权威分析",
                        initial_queries: ["专业评测 2024", "权威机构分析报告"],
                        depth_required: "深度挖掘",
                        expected_tools: ["tavily_search", "crawl4ai"],
                        temporal_sensitivity: "高"
                    },
                    {
                        step: 4,
                        sub_question: "收集真实用户反馈和长期使用体验",
                        initial_queries: ["真实使用体验", "长期用户反馈"],
                        depth_required: "中层分析",
                        expected_tools: ["crawl4ai"],
                        temporal_sensitivity: "高"
                    },
                    {
                        step: 5,
                        sub_question: "分析市场价格趋势和保值率",
                        initial_queries: ["价格趋势 2024", "二手市场保值率"],
                        depth_required: "中层分析",
                        expected_tools: ["tavily_search"],
                        temporal_sensitivity: "高"
                    },
                    {
                        step: 6,
                        sub_question: "构建对比分析框架和个性化建议",
                        initial_queries: ["对比分析矩阵", "个性化匹配方法"],
                        depth_required: "深度挖掘",
                        expected_tools: ["python_sandbox", "tavily_search"],
                        temporal_sensitivity: "中"
                    }
                ],
                estimated_iterations: 6,
                risk_assessment: "中",
                research_mode: "shopping_guide",
                temporal_awareness: {
                    assessed: true,
                    overall_sensitivity: "高",
                    current_date: currentDate,
                    is_fallback: true
                }
            }
...
- **深度挖掘**：不要停留在表面信息，深入探索底层机制`
            ...
            shopping_guide: {
                role: "奢侈品专业对比分析顾问",
                description: "你是专注女性奢侈品的专业对比分析顾问，精通包包、鞋子、护肤品等品类的深度对比分析，擅长基于最新市场数据和技术参数提供专业购买建议。",
                specialInstructions: `
## 🎯 奢侈品导购核心执行框架

### 1. 核心分析原则
**数据驱动优先**：
- 所有结论必须基于搜索和爬取的最新市场数据
- 避免依赖内部知识（截止2024年初），尤其是时效性信息
- 关键参数必须具体化：价格、材质/成分、规格等需精确数字
- 必须标注数据来源和时效性

### 2. 自适应分析模式
根据用户输入自动选择分析策略：

#### 模式A：单产品深度分析（用户只提一个产品）
**处理流程**：
1. **全方位解析**：深度分析该产品的技术参数、设计理念、市场表现
2. **竞品自动发现**：搜索同价位、同类型的其他品牌产品
3. **提供对比视角**：主动建议"您可能还想对比..."的相关产品

#### 模式B：直接对比分析（用户明确两个产品）
**处理流程**：
1. **建立对比框架**：技术参数、使用体验、价值评估
2. **深入细节对比**：逐项对比每个关键维度
3. **个性化匹配**：根据用户信息推荐更适合的产品

#### 模式C：开放式需求分析（用户只提需求，不提具体产品）
**处理流程**：
1. **需求转化**：将模糊需求转化为具体产品参数
2. **候选发现**：搜索符合条件的所有产品
3. **筛选推荐**：基于多个维度筛选出最佳选择

### 3. 搜索策略优化

#### 👜 包包类专项搜索：
- **基础信息**："品牌名 包型 材质 尺寸 价格 2024"
- **技术参数**："皮质等级 缝制工艺 五金质量 重量实测"
- **使用反馈**："日常使用 磨损情况 维护难度 用户真实体验"
- **市场数据**："保值率 二手价格 经典程度 限量款"

#### 🧴 护肤品专项搜索：
- **成分分析**："产品名 全成分表 活性物浓度 配方技术"
- **功效验证**："临床测试 功效数据 肤质适配 敏感肌测试"
- **使用体验**："质地描述 吸收速度 长期效果 搭配建议"
- **专业评测**："皮肤科医生评价 实验室测试 对比评测"

### 4. 数据收集优先级

#### 第一优先级：官方技术参数
- **目标**：品牌官网、官方授权经销商
- **内容**：材质说明、成分表、规格参数、价格信息
- **验证**：必须是官方或授权页面

#### 第二优先级：专业评测
- **目标**：Vogue、Elle、Harper's Bazaar、Consumer Reports
- **内容**：专业评分、测试数据、编辑评语
- **时效**：优先2023-2024年内容

#### 第三优先级：高质量用户反馈
- **目标**：小红书（长篇图文）、亚马逊（Verified Purchase）
- **内容**：详细使用体验、优缺点、长期使用报告
- **筛选**：优先1000字以上、有图片、有时间跨度的内容
...
#### 单产品分析：
- 必须包含：技术参数解析、设计理念分析、市场表现评估
- 建议包含：竞品对比视角、适用场景分析、个性化匹配建议

#### 对比分析：
- 必须建立对比矩阵，涵盖：价格、材质/成分、设计、实用、价值
- 必须包含至少5个专业数据源：官方参数 + 2个专业评测 + 2个真实用户反馈
- 必须进行分场景分析：日常通勤、正式场合、休闲旅行等

### 6. 个性化匹配框架

#### 基于肤质的护肤品匹配：
- **干性皮肤**：重点关注保湿成分、油脂含量、封闭性
- **油性皮肤**：关注控油成分、清爽质地、不致痘配方
- **混合皮肤**：分区护理建议、T区控油/两颊保湿
- **敏感皮肤**：避开刺激性成分、寻找修复舒缓成分

#### 基于使用场景的包包匹配：
- **日常通勤**：尺寸适中、耐磨损、内部结构合理
- **正式场合**：设计经典、品牌辨识度、精致细节
- **休闲旅行**：轻便、耐用、多功能、易搭配

### 7. 风险识别系统

#### 必须识别的风险类型：
1. **质量风险**：产品的常见缺陷、投诉率高的问题
2. **适配风险**：可能不适合特定肤质/体型的情况
3. **潮流风险**：设计可能很快过时、时尚周期短
4. **购买风险**：假货泛滥、渠道不可靠、价格虚高
...
#### 验证方法：
1. **官方数据验证**：至少2个官方渠道确认技术参数
2. **评测交叉验证**：对比3个以上专业评测的观点
3. **用户反馈验证**：寻找多个用户的一致性描述
4. **市场价格验证**：多个平台的价格数据对比
...
1. **执行摘要**：300-500字浓缩结论
2. **需求解析**：明确分析主体和用户诉求
3. **技术调研**：基于搜索结果的产品深度解析
4. **对比分析**：数据驱动的对比矩阵和深入分析
5. **个性化匹配**：基于用户具体需求的适配度评估
6. **购买建议**：明确的推荐理由和购买策略
7. **使用指南**：保养维护和风险规避建议

#### 对比矩阵规范：
\`\`\`markdown
| 对比维度 | 产品A | 产品B | 关键差异 | 胜出方 |
|----------|-------|-------|----------|--------|
| 价格 | ¥28,000 | ¥32,000 | 价格差14% | 产品A |
| 材质 | 小牛皮 | 小羊皮 | 牛皮更耐用 | 产品A |
| 设计 | 经典优雅 | 时尚前卫 | 风格差异 | 依偏好 |
\`\`\`
...
根据实际收集到的信息动态调整：
- **信息丰富时**：深入技术细节，扩展对比维度
- **信息有限时**：聚焦关键参数，提供最核心的建议
- **数据冲突时**：说明数据差异，提供保守建议

**核心目标**：在数据驱动的基础上，提供专业、实用、个性化的奢侈品购买建议。
`
            },
            academic: {
                role: "学术论文深度分析专家",
...
`;
    ...
    _getLuxuryBrandKeywords() {
        return {
            ...
            // 🧴 护肤品品牌
            skincare: [
                'lamer', 'helenarubinstein', 'la prairie', 'sisley',
                'skii', 'cle de peau', 'shiseido', 'estee lauder',
                'lancome', 'chanel', 'dior', 'guerlain', 'fresh',
                'clarins', 'kiehls', 'origins', 'avene', 'la roche-posay',
                '海蓝之谜', '赫莲娜', '莱珀妮', '希思黎',
                'skii', '肌肤之钥', '资生堂', '雅诗兰黛',
                '兰蔻', '香奈儿', '迪奥', '娇兰', '馥蕾诗'
            ],
            ...
            // 👠 鞋子品牌
            shoes: [
                'christianlouboutin', 'manoloblahnik', 'jimmychoo',
                'roger vivier', 'salvatore ferragamo', 'tods',
                'valentino', 'prada', 'gucci', 'chanel',
                '罗杰·维维亚', '菲拉格慕', '托德斯', '华伦天奴'
            ]
        };
    }

    // 🎯 核心方法：判断是否为单产品请求
    _isSingleProductRequest(topic) {
        const lowerTopic = topic.toLowerCase();
        
        // 检测对比关键词
        const comparisonKeywords = ['vs', '对比', '比较', '和', '还是', '哪个好', '与'];
        const hasComparison = comparisonKeywords.some(keyword => lowerTopic.includes(keyword));
        
        if (hasComparison) {
            return false; // 有对比关键词，不是单产品
        }
        
        // 检测品牌数量
        const brandKeywords = this._getLuxuryBrandKeywords();
        const allBrands = [
            ...brandKeywords.bags,
            ...brandKeywords.skincare,
            ...brandKeywords.shoes
        ];
        ...
        // 单产品判断：没有对比关键词且只提及一个品牌
        return !hasComparison && mentionedBrands <= 1;
    }
    // 🎯 新增：品类识别方法
    _detectProductCategory(topic) {
        const lowerTopic = topic.toLowerCase();
        
        // 包包关键词
        const bagKeywords = ['包', '包包', 'bag', 'handbag', 'tote', '背包', '手提包', '斜挎包'];
        const isBag = bagKeywords.some(keyword => lowerTopic.includes(keyword));
        
        // 护肤品关键词
        const skincareKeywords = ['面霜', '精华', '眼霜', '乳液', '护肤', 'skincare', 'cream', 'serum'];
        const isSkincare = skincareKeywords.some(keyword => lowerTopic.includes(keyword));
        
        // 鞋子关键词
        const shoeKeywords = ['鞋', '鞋子', '高跟鞋', '平底鞋', '靴子', 'shoe', 'heel'];
        const isShoe = shoeKeywords.some(keyword => lowerTopic.includes(keyword));
        ...
        // 默认根据品牌判断
        const brandKeywords = this._getLuxuryBrandKeywords();
        if (brandKeywords.bags.some(brand => lowerTopic.includes(brand))) return 'bags';
        if (brandKeywords.skincare.some(brand => lowerTopic.includes(brand))) return 'skincare';
        if (brandKeywords.shoes.some(brand => lowerTopic.includes(brand))) return 'shoes';
