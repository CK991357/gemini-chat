...
- 需要获取完整内容而非摘要时
- **重要提示**: `crawl4ai` 的返回结果（观察）通常是一个经过优化的**智能摘要**，它可能已经包含了你需要的所有结构化信息（如表格）。在编写下一步的`python_sandbox`代码时，**你应该优先尝试从这个摘要中提取数据**，因为它比解析原始HTML更简单、更可靠。只有当摘要信息确实不足时，才需要考虑处理更原始的数据。
- 信息片段不足以回答深度问题时
- **必须参数**：{url: "具体的URL链接"}
