...
4. **发布时间近**：包含 \`2024\`、\`2025\`、\`2026\` 等年份，或 \`latest\`、\`recently\`
...
### 原因
1. **冗余工作**: 最终的写作模型可以通过Data Bus**直接阅读全文**并提取信息
2. **工具限制**: \`code_generator\` 在处理长文本时容易失败（代码错误、输出问题）
3. **效率低下**: 提取工作消耗宝贵迭代次数，却没有增加新信息

### 正确做法
当已通过 \`crawl4ai\` 获取长篇文章内容（>1000字符）时：

1. **记录发现**: 在思考中记录"已获得相关文章，等待最终整合"
2. **继续研究**: 推进到研究计划的**下一个步骤**
3. **信任写作模型**: 相信最终的写作模型会处理Data Bus中的内容

### 错误示例 ❌
\`\`\`
思考: 我需要从文章中提取前三名公司名称...
行动: code_generator
行动输入: {"objective": "从文章中提取公司名称", "data_context": "文章内容..."}
\`\`\`

### 正确示例 ✅
\`\`\`
思考: 已获得相关文章，其中应包含所需信息。我将继续下一步研究。
行动: tavily_search
行动输入: {"query": "下一步研究方向", "max_results": 10}
\`\`\`

### 特殊情况处理
**当以下条件满足时之一，才考虑信息提取**：
1. 需要提取的信息是**结构化数据**（表格、JSON等）
2. 需要**立即使用**该关键数据进行计算或分析
3. 需要调用代码解释器制作图表等可视化素材提供给用户，如果不执行**会无法完成用户的需求**

### 📍 特别注意：代码生成中的标点规范
**如果你必须使用 \`code_generator\`，请严格遵守：**
...
2. **字符串内部的中文文本保持原样**：
   \`\`\`python
   # ✅ 正确：字符串外的标点是英文，字符串内的中文标点不变
   text = "这是一段中文，包含中文标点。"
   
   # ❌ 错误：在代码语法中使用中文标点
   result = {"公司"："编程猫"，"排名"：1}  # 中文冒号、逗号
   \`\`\`

3. **JSON格式必须使用英文标点**：
   \`\`\`python
   # ✅ 正确
   result = {"company": "编程猫", "rank": 1}
...
- **强制任务**:
    ...
    2.  从列表中**识别出 1-4 个最权威、最相关的 URL**。优先选择官方文档、深度文章、研究报告或标题与子问题高度匹配的链接。
    3.  你的下一步行动**必须是**调用 \`crawl4ai\` 来获取这些URL的**完整内容**。
    
    **核心增强：批量爬取 (Batch Crawl)**
    - **模式**: \`batch_crawl\`
    - **用途**: 如果你识别出 **2-4 个**权威 URL，请使用此模式一次性抓取，以加速信息收集。
...
- **可选行动**:
    - 对抓取到的文本进行数据分析、提取或可视化处理。备选工具： \`python_sandbox\` 。
...
${noExtractionProtocol} // 🚫 新增：禁止中间步骤信息提取协议
...
`;
    ...
    // 🎯 增强1：转小写，确保比较公平
    const lowerKeywords1 = keywords1.map(kw => kw.toLowerCase());
    const lowerKeywords2 = keywords2.map(kw => kw.toLowerCase());
    
    // 🎯 增强2：计算精确匹配
    const set1 = new Set(lowerKeywords1);
    const set2 = new Set(lowerKeywords2);
    const exactMatches = [...set1].filter(x => set2.has(x)).length;
    
    // 🎯 增强3：计算部分匹配（包含关系）
    let partialMatches = 0;
    lowerKeywords1.forEach(kw1 => {
        if (lowerKeywords2.some(kw2 => {
            // 双向部分匹配检查
    ...
    lowerKeywords1.forEach(kw1 => {
        lowerKeywords2.forEach(kw2 => {
            // 检查是否互为同义词
            if (synonymMap[kw1] && synonymMap[kw1].includes(kw2)) {
                semanticMatches += 1.5; // 语义匹配权重更高
            } else if (synonymMap[kw2] && synonymMap[kw2].includes(kw1)) {
                semanticMatches += 1.5;
            }
        });
    });
    ...
    // 🎯 增强5：加权计算总分
    const totalPossible = Math.max(keywords1.length, keywords2.length);
    
    const exactScore = exactMatches / totalPossible; // 精确匹配权重最高
    const partialScore = partialMatches / (totalPossible * 2); // 部分匹配权重减半
    const semanticScore = semanticMatches / (totalPossible * 3); // 语义匹配权重适中
    
    // 综合得分（可根据需要调整权重）
    return (exactScore * 0.6) + (partialScore * 0.3) + (semanticScore * 0.1);
}
    ...
    if (similarity > 0.8) {
        return `🔥 高度相似（${percent}%），强烈建议复用历史步骤${toolName ? `的\`${toolName}\`调用` : ''}，补充新角度`;
    } else if (similarity > 0.6) {
        return `⚠️ 中度相似（${percent}%），可参考历史${toolName ? `\`${toolName}\`` : ''}方法，但需要新信息验证`;
    } else if (similarity > 0.4) {
        return `🔍 轻度相似（${percent}%），可快速浏览历史，但建议执行新搜索`;
    } else {
        return `📝 低度相似（${percent}%），建议执行全新搜索`;
    }
}
