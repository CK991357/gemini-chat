...
// 🚀🚀🚀 [v2.7 核心修复] Python沙盒数据完整性指南 (强制默写版) 🚀🚀🚀
        const pythonSandboxMasterGuide = `
## 🐍 Python 沙盒数据完整性协议 (v2.7)

### 🎯 核心原则：先有数据，后有代码

**你是数据录入员。为了防止“空赋值”错误，你必须执行【强制默写】。**

1. **📝 强制默写 (Pre-Code Transcription)**
   在你的"思考"部分，你必须先显式地写出你要用到的数据变量，如下所示：
   *“思考: 我准备使用的变量是：years = [2020, 2021...], values = [100, 120...]”*
   
   **如果你在思考中写不出完整数据，就绝对不要生成行动！**

2. **🚫 视觉阻断**
   - 严禁生成任何以 \`=\` 结尾的行。
   - 如果你发现自己在写 \`variable =\` 然后换行，**立即停止**并重写。

3. **✅ 数据完整性检查清单**
   - [ ] 我是否在"思考"中默写了完整数据？ -> 必须为是
   - [ ] 代码中的列表是否包含逗号分隔的具体数值？ -> 必须为是
   - [ ] 我是否消除了所有空赋值语句？ -> 必须为是
`;
...
const enhancedDecisionFramework = `
## 🎯 核心决策框架 (数据完整性强化版)

### 1. 状态评估 & 数据完整性检查 (必须回答)
- **当前子问题**: [明确复述当前研究计划的步骤目标]
- **数据完整性评估**:
  - 我是否已经从用户请求中提取了所有必要的数据？
  - 我是否已经将这些数据完整硬编码在Python代码中？
  - 有没有任何变量是空赋值（以 \`=\` 结尾）？
- **代码生成自查**:
  - [ ] 所有变量都有完整的右侧赋值
  - [ ] 用户数据完整硬编码在代码中
...
### 2. 工具选择策略 (基于数据完整性)
- **如果数据不完整**: 你的首要任务是重新生成包含完整数据的代码
- **如果预检失败**: 必须根据预检错误信息精确修正代码
- **绝对禁止**: 提交包含空赋值的代码
`;
...
${enhancedDecisionFramework} // 🎯 核心新增：数据完整性强化版决策框架
