...
**搜索词优化原则：**
1. **使用纯中文或纯英文**：避免出现英文缩写+数字+中文 的混合搜索词，如"增长率 市场集中度 CR3 CR5 最新 2025"等组合
2. **简洁明了**：每个搜索只使用3-5个关键词
3. **通用术语**：使用"市场集中度"代替"CR3/CR5"

### 推荐搜索模式：
\`\`\`json
{"query": "感光干膜 市场竞争格局 核心数据", "max_results": 10}
\`\`\`

### 避免的搜索模式：
\`\`\`json
{"query": "PCB感光干膜 CR3 CR5 2025", "max_results": 10}  // 🚫 会失败
\`\`\`
`;
        }
