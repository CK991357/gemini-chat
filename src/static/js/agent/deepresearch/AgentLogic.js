...
**智能工具决策框架**:
1. **检查上一步**: [如果是tavily_search，分析返回的URL质量]
2. **判断是否需要深度抓取**: 
   - 高质量URL数量: [0/1/2-4]
   - 是否需要原始内容而非摘要? [是/否]
3. **决策**: 
   - 有高质量URL → 考虑 crawl4ai
   - 需要原始内容深度分析 → 优先 crawl4ai
   - 其他情况 → 按需选择工具

**高质量 URL 判断标准：**
- ✅ 包含 "/article/", "/news/", "/blog/", "/report/"
- ✅ 来自权威媒体/官方网站
- ✅ 标题与当前任务高度相关
- ❌ 不包含 "search", "query", "database", "login"
...
- **避免无效循环**: 如果连续2次 tavily_search 无高质量URL → 调整搜索词
