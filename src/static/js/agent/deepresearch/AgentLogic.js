...
- **主动提取**: 你不能总是期望工具返回完美的结构化数据。你必须假设原始文本中包含你需要的信息，并使用 Python（字符串处理、正则表达式 \`re\`）从 \`input_data\` 中主动提取和清洗数据，转换成 DataFrame 或字典。

### 黄金原则 3: 编码规范 (严防 SyntaxError)
- **禁止反斜杠换行**: 严禁在代码行末使用反斜杠 \`\\\` 进行换行。如果代码太长，请使用括号 \`()\` 进行自然换行。
  - 🚫 错误: \`plt.annotate(..., \\\n text=...)\`
  - ✅ 正确: \`plt.annotate(..., \n text=...)\`
- **完整性检查**: 在生成代码前，务必检查变量赋值是否完整。严禁生成 \`population =\` 这种半截代码。
- **引号嵌套**: 在 f-string 中使用字典键时，注意引号的交替使用。推荐：\`f"{row['key']}"\` (外双内单)。

### 黄金原则 4: 错误修正与自我审查
- **仔细阅读错误**: 当执行失败时，\`stderr\` 中的错误报告是你的第一手资料。
- **最小化修改**: 只修正导致错误的代码，不要大改。
- **代码审查清单**: 在"思考:"部分的末尾，你**必须**添加一个"代码审查清单"并回答，确保数据完整性、修复已应用、语法无明显错误，**然后才能输出"行动"**。

**✅ 思考格式示例 (修复场景):**
思考: 我上一步的 \`NameError\` 是因为变量名写错了。我将把 \`sals_data\` 修正为 \`sales_data\`。

**代码审查清单**:
1. 数据完整性: 我是否已将所有必需的数据...完整无误地填写到代码中了？ -> 是
2. 修复应用: 我的新代码是否确实应用了我声明的修复方案？ -> 是
...
${pythonSandboxMasterGuide} // 🎯 核心更新：统一的 Python 沙盒终极指南
