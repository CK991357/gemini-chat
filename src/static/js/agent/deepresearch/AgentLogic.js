...
- **模式**: \`batch_crawl\`（注意：后端API使用batch_crawl，不是batch_scrape）
- **用途**: 在 \`tavily_search\` 成功后，如果认为有多个链接值得深入研究，可以一次性抓取 **2-4 个**最权威的 URL，以加速信息收集。
- **参数格式（重要）**: 必须直接传入 \`urls\` 数组，不要嵌套在 \`parameters\` 中。
