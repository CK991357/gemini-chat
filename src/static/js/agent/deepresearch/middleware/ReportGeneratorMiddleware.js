...
- **数据呈现表格化:** 涉及数据对比、性能指标、分类汇总、参数说明等内容时，**必须优先使用表格呈现**。即使原始数据是文字描述，也要将其整理成结构化的表格格式，以提升报告的专业性和可读性。表格应有清晰标题，数据对齐（数值右对齐，文本左对齐），关键数据用**加粗**突出显示。
   - **数据优先原则**: 在每章节中，先罗列关键数据（用表格形式），再进行文字分析
   - **表格化场景**: 对比分析、性能测试、优缺点总结、分类说明、参数对比等
   - **格式要求**: 数值右对齐(\`:--:\`)，文本左对齐(\`:--\`)，关键行/列加粗
   - **最少表格**: 报告中至少包含2-3个专业表格
...
4. **学术引用**：严格按照引用规范标注来源
5. **数据表格化:** 对比性数据、性能指标、分类汇总等信息**必须使用表格呈现**。先罗列数据表格，再进行文字分析。确保表格有清晰标题，数据对齐（数值右对齐: \`:--:\`，文本左对齐: \`:--\`），关键数据突出显示。
                    ...
                    switch(dataStrategy) {
                        case 'full_original':
                            if (originalDataForProcessing.length < 80000) {
                                ...
                                dataSourceType = 'data_bus_full_cleaned';
                                ...
                                dataSourceType = 'data_bus_full_summary';
                            }
                            break;
                        ...
                        case 'enhanced_summary':
                            ...
                            dataSourceType = 'data_bus_enhanced_summary';
                            break;
