...
export class DeepResearchAgent {
    ...
    async conductResearch(researchRequest) {
        ...
        // 🎯 存储报告模型选择
        if (reportModel) {
            this.reportGenerator.reportModel = reportModel;
        }
        ...
        this.stateManager.clearImages(); // 清空图片缓存
        ...
        try {
            ...
            await this.callbackManager.invokeEvent('on_research_plan_generated', {
                ...
                data: {
                    ...
                    plan_text: JSON.stringify(researchPlan, null, 2),
                    plan_object: researchPlan,
                    keywords: [],
                    ...
                    temporal_awareness: researchPlan.temporal_awareness
                }
            });
        ...
        while (iterations < this.maxIterations && consecutiveNoGain < noGainThreshold && !finalAnswerFromIteration) {
            ...
            const planCompletion = this._calculatePlanCompletion(researchPlan, this.intermediateSteps);
            ...
            try {
                ...
                this.lastDecisionText = agentDecisionText;
                this._updateTokenUsage(agentDecision.usage);
                ...
                // 🎯 处理报告大纲生成
                if (parsedAction.type === 'generate_outline' || parsedAction.tool_name === 'generate_outline') {
                ...
                if (parsedAction.type === 'tool_call') {
                    ...
                    await this.callbackManager.invokeEvent('on_tool_end', {
                        ...
                        data: {
                            ...
                            sources_found: toolSources.length,
                            success: toolSuccess,
...
`;
                        this.lastDecisionText = correctionPrompt;
                        parserErrorOccurred = true;
                        this.parserRetryAttempt = 1;
    ...
    _updateTokenUsage(usage) {
        if (!usage) return;
        this.stateManager.updateMetrics({ tokenUsage: usage });
