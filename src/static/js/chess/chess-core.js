...
class ChessGame {
   ...
   completelyResetGame() {
    ...
    /**
     * 添加控制按钮（整合AI按钮和保存加载按钮）
     */
    addControlButtons() {
        const fenActions = document.querySelector('.fen-actions');
        if (!fenActions) {
            console.error('.fen-actions container not found for control buttons.');
            return;
        }

        // 清空容器，防止重复添加
        fenActions.innerHTML = '';
        ...
        const buttons = [
            ...
            { 
                id: 'ask-ai-best-move-button', 
                icon: 'fa-brain', 
                text: '问AI最优解', 
                title: '请求AI进行详细局面分析', 
                handler: () => this.handleAskAIBestMove() 
            },
            // 新增的保存和加载按钮
            { 
                id: 'save-game-button', 
        ...
        // 重新附加原有的按钮事件（如果它们被清空了）
        this.attachOriginalButtonEvents();
    }

    /**
     * 重新附加原有按钮事件
     */
    attachOriginalButtonEvents() {
        const copyButton = document.getElementById('copy-fen-button');
        const resetButton = document.getElementById('reset-chess-button');
        const undoButton = document.getElementById('undo-move-button');
