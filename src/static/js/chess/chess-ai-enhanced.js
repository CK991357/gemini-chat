...
2.  **逗号分隔格式**：将所有找到的SAN走法以一个半角逗号分隔的列表形式返回。例如："Nf6, Qe7"。
3.  **只返回SAN列表**：你的输出必须是且仅是这个逗号分隔的SAN字符串列表。不要添加任何解释、编号、前缀或多余的文字。
4.  **忽略负面走法**：不要提取那些被评价为“不可取”、“劣势”或仅用于分析目的的走法。
...
**你的输出应该是一个逗号分隔的SAN字符串列表，例如 "Nf6, Qe7"。**`;
    }
