...
基于前后端代码分析，我为你编写了详细的 Crawl4AI 工具使用指南：
...
| \`extract\` | 结构化数据提取 | 基于CSS或LLM提取特定数据 |
| \`batch_crawl\` | 批量URL处理 | 同时处理多个URL |
| \`pdf_export\` | PDF导出 | 将网页导出为PDF |
...
- \`css_selector\`: 提取特定内容的CSS选择器
- \`return_screenshot\`: 是否返回截图(base64)
- \`return_pdf\`: 是否返回PDF(base64)
- \`screenshot_quality\`: 截图质量(10-100)，默认70
- \`screenshot_max_width\`: 截图最大宽度，默认1920
- \`word_count_threshold\`: 内容块最小单词数，默认10
- \`exclude_external_links\`: 是否排除外部链接，默认true

#### 2. 深度网站爬取 (\`deep_crawl\`)

使用智能策略深度爬取整个网站，支持关键词评分和URL过滤。
...
- \`include_external\`: 是否跟踪外部链接，默认false
- \`keywords\`: 用于相关性评分的关键词列表
- \`url_patterns\`: URL模式过滤列表
- \`stream\`: 是否流式返回结果，默认false

#### 3. 批量URL处理 (\`batch_crawl\`)
...
- \`concurrent_limit\`: 最大并发数，默认3

### 4. 结构化数据提取 (\`extract\`)

从网页中提取结构化数据，支持CSS选择器和LLM智能提取。
...
**参数说明:**
- \`url\` (必需): 要提取的网页URL
- \`schema_definition\` (必需): 定义输出结构的JSON schema
- \`css_selector\`: 基础CSS选择器（CSS提取时使用）
...
**参数说明:**
- \`url\` (必需): 要导出为PDF的网页URL
- \`return_as_base64\`: 是否返回base64编码，默认true
...
- \`return_as_base64\`: 是否返回base64编码，默认true
- \`quality\`: 截图质量(10-100)，默认70
- \`max_width\`: 最大宽度，默认1920
- \`max_height\`: 最大高度，默认5000
...
这个指南基于crawl4ai主程序前后端代码编写，确保了参数结构和功能与实现完全匹配。模型可以按照这个指南正确调用 Crawl4AI 的所有功能。
`
        },
                {
            id: 'Tool_gemini',
